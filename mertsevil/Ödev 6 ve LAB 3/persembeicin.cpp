...
int main(){
	...
	int satirsayisi, sutunsayisi,i,j,matris[31][31],k,temp,bagintimatrisi[31][31],a=0,deger,b,bulunan,secim,dugumlermatrisi[100][3],s=0,renk=0,sakliadres=0,h=2,g,color=45,ilksefer=0,gun=0,l;
	printf("Merhaba");
 ...
 int ematris[6][9] = { {11 , 13 , 15 , 17 , 19 , 21 , 23 , 25 , 33 }, {11 , 14 , 16 , 20 , 22 , 24 , 26 , 35 , 43} , {1 , 17 , 24 , 31 , 33 , 35 , 39 , 43 , 45 } , { 32 , 34 , 36 , 38 , 39 , 42 , 44 , 46 , 48 } , { 16 , 38 , 43 , 51 , 57 , 61 , 63 , 45 , 58} , {34 , 54 , 57 , 58 , 59 , 63 , 64 , 71 , 72 } };
	satirsayisi=6;
	sutunsayisi=9;
		for (i=0;i<satirsayisi;i++){
	   for (j=0;j<sutunsayisi;j++){
...
//printf("Satir no guncellendi artik %d .satira bakiliyor\n",i);
	...
	printf("\n");
	   ...
	   for (j=1;j<4;j++){
			...
			//	 printf("Dugum Numarasi %d:%d        ",a,bagintimatrisi[i][j]);
			if(j==1 || j==2)
			printf("%d                           ",dugumlermatrisi[i][j]+1);
			if(j==3)
	...
	for(i=0;i<satirsayisi;i++){
		for(j=0;j<sutunsayisi;j++){
		for(k=0;k<satirsayisi;k++){
			for(l=0;l<sutunsayisi;l++){
				if(matris[i][j]==matris[k][l] && i!=k && j!=l){
					printf("%d                        %d                         %d\n",i+1,k+1,matris[k][l]);
				}
...
//gunler[6][9] ="Pazar";
	...
	gun=0;
	printf("\n");	
	printf("Gunlere gore sinav dagilimlari graf boyama algoritmasi sonuclarina gore asagidaki gibidir\n");
	printf("\n");	
	...
	for(i=0;i<satirsayisi;i++){
		for(j=0;j<satirsayisi;j++){
		for(k=0;k<satirsayisi;k++){
			for(l=0;l<satirsayisi;l++){
				if(i==j && k==l){
			if(bagintimatrisi[i][j]==bagintimatrisi[k][l] ){
		...
		if(i!=k){
		printf("%d. ders %d. ders ile ayni gun yapilabilir ve ders %s olabilir\n",i+1,k+1,gunler);
	
     	}
     	else{
     	printf("%d. dersin %s gunu yapildigi varsayimi kabul edilsin \n",i+1,gunler);
        }	
		printf("\n");
	                 }	
